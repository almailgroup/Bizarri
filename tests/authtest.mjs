...
const session = (id) => ({
  access_token: "tok-" + id, token_type: "bearer", expires_in: 3600,
  expires_at: Math.floor(Date.now() / 1000) + 3600, refresh_token: "r-" + id,
...
await ctx.route(SUPA, async (route) => {
  ...
  if (rest === "rpc/is_admin") { state.isAdminCalls++; return send(state.admin); }
  if (rest === "chalets") return send([{ id: 1, slug: "b1", name_en: "Bizarri Chalet 1", name_ar: "١", active: true, sort_order: 1 }]);
  if (rest === "rates") return send({ id: true, full_week: 600, weekend: 350, weekday: 300, daily_weekday: 75, daily_weekend: 120, currency: "KWD", min_stay_days: 3, updated_at: "", updated_by: null });
  if (rest === "bookings") {
    state.bookingCalls++;
    return send(state.admin ? [{ id: "b1", ref: "BZR-SECRET", chalet_id: 1, start_date: "2026-10-11",
      end_date: "2026-10-17", days: 7, total: 600, currency: "KWD", package_key: "fullWeek",
      guest_name: "Confidential Guest", guest_phone: "+96599999999", guest_email: "private@example.com",
      guests: 4, notes: null, status: "pending", admin_note: null, created_at: new Date().toISOString(),
      updated_at: "", decided_at: null, decided_by: null }] : []);
  }
  return send([]);
});
...
const sawDashboard = await p.getByText("Availability & Pricing").isVisible().catch(() => false);
const sawDenied = await p.getByText(/does not have chalet management access/i).isVisible().catch(() => false);
ck("Non-admin is refused, not shown a cached dashboard", !sawDashboard && sawDenied,
...
ck("is_admin was re-checked for the new user", state.isAdminCalls >= 2, `${state.isAdminCalls} calls`);
